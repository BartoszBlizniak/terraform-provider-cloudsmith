...
The provider reads the identity token from `TFC_WORKLOAD_IDENTITY_TOKEN_CLOUDSMITH`, then `TFC_WORKLOAD_IDENTITY_TOKEN`. It POSTs that token to `https://api.cloudsmith.io/openid/{organization}/` and uses the returned Cloudsmith JWT as `X-Api-Key`. The JWT is refreshed a minute before its `exp` claim, or every 15 minutes if it carries no readable expiry. If a request still comes back 401, the provider discards that token, exchanges a fresh one, and replays the request once.
